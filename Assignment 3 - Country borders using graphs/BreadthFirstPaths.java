...
public class BreadthFirstPaths {
	 ...
	 */
	public BreadthFirstPaths(AdjListGraph G, int s, ArrayList<Integer> blockedCountries) {
		marked = new boolean[G.V()];
		distTo = new int[G.V()];
		edgeTo = new int[G.V()];
		blocked = new boolean[G.V()];
		for(int i = 0; i<blocked.length; i++) {
			blocked[i] = false;
		}

		for(int i = 0; i<blockedCountries.size(); i++) {
			blocked[blockedCountries.get(i)] = true;
		}
		tempQ = new Queue<Integer>();
	...
	private void bfs(AdjListGraph G, int s) {
	...
	public Queue<Integer> returnQueue() {
	...
	private boolean check(AdjListGraph G, int s) {
